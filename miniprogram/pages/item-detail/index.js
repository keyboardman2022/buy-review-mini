...
Page({
  ...
  async react(event) { if (this.data.reacting) return; const value = Number(event.currentTarget.dataset.value); const next = this.data.item.myReaction === value ? 0 : value; this.setData({ reacting: true }); try { await request(`/api/items/${this.data.id}/reaction`, { method: 'POST', data: { value: next } }); await this.load(); } catch (error) { this.setData({ error: error.message }); } finally { this.setData({ reacting: false }); } },
  editItem() { wx.navigateTo({ url: `/pages/compose/index?id=${this.data.id}` }); },
  createApproval() { wx.navigateTo({ url: `/pages/approval-create/index?itemId=${this.data.id}` }); },
  deleteItem() { wx.showModal({ title: '移出购物车？', content: '此操作不会删除已有审批快照。', confirmColor: '#D95F45', success: async ({ confirm }) => { if (!confirm) return; try { await request(`/api/items/${this.data.id}`, { method: 'DELETE' }); wx.switchTab({ url: '/pages/cart/index' }); } catch (error) { this.setData({ error: error.message }); } } }); },
});
